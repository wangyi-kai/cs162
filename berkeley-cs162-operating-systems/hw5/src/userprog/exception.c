...
page_fault (struct intr_frame *f)
{
	...
	if (user) {
		if (fault_addr >= f->esp - 32) {
			size_t num_pages = (size_t)((void *)t->stack_page - pg_round_down(fault_addr)) / PGSIZE;
			void *paddr = palloc_get_multiple(PAL_ZERO | PAL_USER, num_pages);
			if (paddr == NULL)
				syscall_exit(-1);
			t->stack_page = pg_round_down(fault_addr);
			size_t i;
			for (i = 0; i < num_pages; i++) {
				pagedir_set_page(t->pagedir, t->stack_page + i * PGSIZE, paddr + i * PGSIZE, true);
			}
			return;
		}
		else
